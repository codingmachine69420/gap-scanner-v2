...
# The job's crons start it at ~09:23 ET, so the real wait is ~22 minutes. The
# widest legitimate wait is from the guard's own start (08:30 ET), 75.5
# minutes. The cap only needs to catch genuinely broken time logic -- a
# multi-hour wait -- so it sits just above that.
MAX_SLEEP_SECONDS = 90 * 60
